    ...
    <div class="q-pa-md">
      <Filterable v-bind="filterable" v-slot="{ item }">
        <tr class="text-center cursor-pointer"
          ...
          <td class="border" :hidden="filterable.orderables[2].hidden"> {{ item.dni || '-' }} </td>
          <td class="border" :hidden="filterable.orderables[3].hidden"> {{ item.motivo }} </td>
          <td class="border" :hidden="filterable.orderables[4].hidden"> {{ item.lugar }} </td>
          <td class="border" :hidden="filterable.orderables[5].hidden"> {{ item.hora_ing }} </td>
          <td class="border" :hidden="filterable.orderables[6].hidden">
            <template v-if="item.hora_sal">
              {{ item.hora_sal }}
            </template>
            <template v-else>
              <q-btn class="bg-deep-purple-6 text-white" size="sm" label="Marcar salida" push @click="handleSalida(item.id)" />
            </template>
          </td>
          <td class="border" :hidden="filterable.orderables[7].hidden"> {{ item.area.nombre }} </td>
          <td class="border" :hidden="filterable.orderables[8].hidden">
            <q-badge outline :color="item.tipo_estado.id === 5 ? 'green' : 'red'" :label="item.tipo_estado.nombre" />
          </td>
          <td class="border" :hidden="filterable.orderables[9].hidden">
            {{ item.user.nombre }}
          </td>
        </tr>
      </Filterable>
...
export default {
  ...
  setup() {
    ...
    const { check_select, filterable, check } = visitas('visitante')
