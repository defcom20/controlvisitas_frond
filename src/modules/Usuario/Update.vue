      ...
      <q-card-section class="bg-white q-gutter-y-sm" rounded>
        ...
        <div class="row q-mt-md">
          <div class="col flex items-center justify-end">
            ...
            <q-btn push class="bg-3" text-color="white" icon="save" label="Actualizar" @click="onSubmit" />
          </div>
        </div>
      </q-card-section>
    </form>
...
export default {
  ...
  setup() {
    ...
    const onSubmit = async () => {
      ...
      const res = await actualizarUser(form, router.currentRoute.value.params.id)
      if (res.success) {
        router.go(-1);
      }
    }
